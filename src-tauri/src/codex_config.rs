...
fn codex_catalog_model_entry(
    ...
    if let Some(upstream) = codex_upstream_model_capabilities(&spec.model) {
        ...
        // 上下文窗口同样以上游为准（模板/预设常年落后于真实值）。
        if let Some(window) = upstream
            .get("context_window")
            .and_then(Value::as_u64)
            .filter(|value| *value > 0)
        {
            entry_obj.insert("context_window".to_string(), json!(window));
            entry_obj.insert("max_context_window".to_string(), json!(window));
        }
    }
...
/// company fork: 从 Codex 的 `models_cache.json` 里按 slug 取某个模型的真实能力。
///
/// 该文件由 Codex 连接 OpenAI 后落盘，逐模型准确（gpt-5.6-sol/terra 有 `max` 与
/// `ultra` 档、luna 只有 `max`，都带 `fast` 速度档与 priority 服务档、272k 上下文）。
/// 用它覆盖模板克隆的结果，官方模型的能力就自动跟随上游，无需在代码里维护档位表。
///
/// 找不到文件、解析失败或没有匹配 slug 时一律返回 `None`，调用方保持既有行为，
/// 因此自定义模型与第三方中转别名不受影响。
fn codex_upstream_model_capabilities(slug: &str) -> Option<Value> {
    let slug = slug.trim();
    if slug.is_empty() {
        return None;
    }

    let catalog = load_codex_upstream_models_cache()?;
    catalog
        .get("models")
...
/// 这里按进程缓存一次。测试绕开缓存，避免相互串味（各用例会写不同的 CODEX_HOME）。
#[cfg(not(test))]
